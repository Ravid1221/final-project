...
} = require("./usersValidations/registraion");
...
router.post("/resetPassword", async (req, res) => {
  ...
  let user = await User.findOne({ email: req.body.email });
  if (!user) {
    console.log(chalk.redBright("Reset Password Error: User doesn't exist"));
  ...
  user.password = generateHashPassword(req.body.newPassword);
  await user.save();
  res.send(_.pick(user, ["_id", "name", "email"]));
});
